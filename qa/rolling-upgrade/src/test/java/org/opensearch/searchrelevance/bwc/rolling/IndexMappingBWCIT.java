 ...
 *
 * This test validates that index mappings are properly updated during rolling upgrades.
 * It simulates a scenario where an index was created with a minimal/old mapping and
 * verifies that the onNodeStarted() mechanism adds new fields after upgrade.
 *
 * Test flow:
 ...
 * 2. MIXED cluster: Validates that data is still accessible during rolling upgrade.
 * 3. UPGRADED cluster: Validates that:
 *    - The mapping was updated with NEW fields (description, timestamp, sampling, querySetQueries)
 *    - Existing data is still accessible
 *    - New documents can be written with the updated mapping
 */
public class IndexMappingBWCIT extends AbstractSearchRelevanceRollingUpgradeTestCase {
    ...
    private static final String TEST_DOC_ID = "bwc-test-doc-1";
    ...
    /**
     * Main BWC test for Index Mapping Update functionality.
     */
    public void testIndexMappingUpdate_RollingUpgrade() throws Exception {
        switch (getClusterType()) {
            case OLD:
                ...
                break;
            case MIXED:
                testValidateDataInMixedCluster();
                break;
            case UPGRADED:
     ...
     * This simulates what would happen if an index was created with an older plugin version
     * that had fewer fields in its mapping.
     */
    ...
    private void testCreateIndexWithMinimalMappingInOldCluster() throws Exception {
        ...
        String docBody = "{\"id\": \"" + TEST_DOC_ID + "\", \"name\": \"BWC Test Document\"}";
        ...
        assertEquals("Document name should match", "BWC Test Document", doc.get("name"));
    }

    /**
     * MIXED cluster test: Validate that data is still accessible during rolling upgrade.
     */
    private void testValidateDataInMixedCluster() throws Exception {
        // Verify the index still exists
        assertTrue("Index should still exist in MIXED cluster", checkIndexExists(QUERY_SET_INDEX));

        // Verify document is still accessible
        Map<String, Object> doc = getDocument(QUERY_SET_INDEX, TEST_DOC_ID);
        assertNotNull("Document from OLD cluster should be accessible in MIXED cluster", doc);
        assertEquals("Document name should still match", "BWC Test Document", doc.get("name"));

        // Log the current mapping
        Map<String, Object> mapping = getIndexMapping(QUERY_SET_INDEX);
        logger.info("MIXED cluster mapping for {}: {}", QUERY_SET_INDEX, mapping);
    }

    /**
     * UPGRADED cluster test: Validate that mappings were updated with new fields.
     * The onNodeStarted() method should have applied the latest mapping from queryset.json.
     */
    @SuppressWarnings("unchecked")
    private void testValidateMappingUpdatedInUpgradedCluster() throws Exception {
        ...
        assertEquals("Document name should still match after upgrade", "BWC Test Document", doc.get("name"));
        ...
        String newDocId = "bwc-test-doc-upgraded";
        String newDocBody = "{"
            + "\"id\": \""
            + newDocId
            + "\","
            + "\"name\": \"Upgraded Test Document\","
            + "\"description\": \"Document created after upgrade with new fields\","
            + "\"sampling\": \"random\","
        ...
        assertEquals("New document name should match", "Upgraded Test Document", newDoc.get("name"));
        assertEquals("New document description should match", "Document created after upgrade with new fields", newDoc.get("description"));
