...
public class RestSearchExperimentActionTests extends OpenSearchTestCase {
    private RestSearchExperimentAction restAction;
    ...
    public void setup() throws IOException {
        ...
        restAction = new RestSearchExperimentAction(settingsAccessor);
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(client).execute(eq(SearchExperimentAction.INSTANCE), any(), any());
    }
    ...
    public void testGetNameAndRoutes() {
        ...
        assertEquals("search_experiment_action", actionName);

        List<RestHandler.Route> routes = restAction.routes();
        assertNotNull(routes);
        assertEquals(2, routes.size());
        assertThat(routes.get(0).getPath(), Matchers.is(EXPERIMENTS_URI + "/_search"));
    ...
    public void testHandleRequest_whenWorkbenchDisabled_returnsForbidden() throws Exception {
        ...
        verify(client, never()).execute(eq(SearchExperimentAction.INSTANCE), any(), any());
        ArgumentCaptor<BytesRestResponse> responseCaptor = ArgumentCaptor.forClass(BytesRestResponse.class);
        verify(channel, times(1)).sendResponse(responseCaptor.capture());
