...
public class SearchQuerySetTransportActionTests extends OpenSearchTestCase {
    ...
    private SearchQuerySetTransportAction transportAction;

    @Before
    public void setup() {
        MockitoAnnotations.openMocks(this);
        transportAction = new SearchQuerySetTransportAction(transportService, actionFilters, querySetDao);
    }
    ...
    public void testDoExecute_withSourceBuilder_callsDaoAndResponds() {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(querySetDao).listQuerySet(eq(builder), any());

        transportAction.doExecute(null, request, listener);

        verify(querySetDao).listQuerySet(eq(builder), any());
    ...
    public void testDoExecute_withoutSourceBuilder_constructsDefault() {
        ...
        doAnswer(invocation -> {
            ...
            l.onResponse(searchResponse);
            return null;
        }).when(querySetDao).listQuerySet(any(SearchSourceBuilder.class), any());

        transportAction.doExecute(null, request, listener);

        ArgumentCaptor<SearchSourceBuilder> captor = ArgumentCaptor.forClass(SearchSourceBuilder.class);
        verify(querySetDao).listQuerySet(captor.capture(), any());
    ...
    public void testDoExecute_whenDaoThrows_returnsFailure() {
        ...
        doThrow(new RuntimeException("boom")).when(querySetDao).listQuerySet(eq(builder), any());
