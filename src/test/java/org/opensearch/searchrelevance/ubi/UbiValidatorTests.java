...
public class UbiValidatorTests extends OpenSearchTestCase {
    ...
    public void testCheckUbiIndicesExistWithNullClusterService() {
        assertFalse(UbiValidator.checkUbiIndicesExist(null));
        assertFalse(UbiValidator.checkUbiIndicesExist(null, "ubi_queries", "ubi_events"));
        assertFalse(UbiValidator.checkUbiQueriesIndexExists(null, "ubi_queries"));
        assertFalse(UbiValidator.checkUbiEventsIndexExists(null, "ubi_events"));
    }

    public void testCheckDefaultUbiIndicesExist() {
        ...
        assertTrue(UbiValidator.checkUbiIndicesExist(clusterService));
    }

    public void testCheckDefaultUbiIndicesNotExist() {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterState clusterState = mock(ClusterState.class);
        Metadata metadata = mock(Metadata.class);
        ...
        assertFalse(UbiValidator.checkUbiIndicesExist(clusterService));
    }

    public void testCheckCustomUbiIndicesExist() {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterState clusterState = mock(ClusterState.class);
        Metadata metadata = mock(Metadata.class);
        ...
        when(clusterService.state()).thenReturn(clusterState);
        when(clusterState.metadata()).thenReturn(metadata);
        when(metadata.hasIndex("custom_queries")).thenReturn(true);
        when(metadata.hasIndex("custom_events")).thenReturn(true);

        assertTrue(UbiValidator.checkUbiIndicesExist(clusterService, "custom_queries", "custom_events"));
    }

    public void testCheckCustomUbiIndicesNotExist() {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterState clusterState = mock(ClusterState.class);
        Metadata metadata = mock(Metadata.class);
        ...
        when(clusterService.state()).thenReturn(clusterState);
        when(clusterState.metadata()).thenReturn(metadata);
        when(metadata.hasIndex("custom_queries")).thenReturn(false);
        when(metadata.hasIndex("custom_events")).thenReturn(true);

        assertFalse(UbiValidator.checkUbiIndicesExist(clusterService, "custom_queries", "custom_events"));
    }

    public void testCheckOnlyQueriesIndexExists() {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterState clusterState = mock(ClusterState.class);
        Metadata metadata = mock(Metadata.class);
    ...
    public void testCheckOnlyEventsIndexExists() {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterState clusterState = mock(ClusterState.class);
        Metadata metadata = mock(Metadata.class);
