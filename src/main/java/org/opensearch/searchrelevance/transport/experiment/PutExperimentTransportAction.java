...
public class PutExperimentTransportAction extends HandledTransportAction<PutExperimentRequest, IndexResponse> {
    ...
    private void triggerAsyncProcessing(String experimentId, PutExperimentRequest request) {
        ...
        querySetDao.getQuerySet(request.getQuerySetId(), ActionListener.wrap(querySetResponse -> {
            try {
                ...
                List<String> queryTextsWithCustomInput = querySet.querySetQueries()
                    .stream()
                    .map(e -> e.queryText())
                    .collect(Collectors.toList());

                // Check if queryTexts is empty and complete experiment immediately
                if (queryTextsWithCustomInput.isEmpty()) {
                ...
                fetchSearchConfigurationsAsync(experimentId, request, queryTextsWithCustomInput);
            } catch (Exception e) {
    ...
    private void fetchSearchConfigurationsAsync(String experimentId, PutExperimentRequest request, List<String> queryTextsWithCustomInput) {
            ...
            searchConfigurationDao.getSearchConfiguration(configId, ActionListener.wrap(searchConfigResponse -> {
                try {
                    ...
                    if (pendingConfigs.decrementAndGet() == 0) {
                        calculateMetricsAsync(experimentId, request, searchConfigurations, queryTextsWithCustomInput);
                    }
                } catch (Exception e) {
    ...
    private void calculateMetricsAsync(
        ...
        if (queryTextsWithCustomInput == null || searchConfigurations == null) {
            throw new IllegalStateException("Missing required data for metrics calculation");
        }

        processQueryTextMetrics(experimentId, request, searchConfigurations, queryTextsWithCustomInput);
    }
